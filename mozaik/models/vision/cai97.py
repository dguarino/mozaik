...
def sRF_2d(x, y, t, p):
    ...
    tmc = G(t, p.K1, p.c1, p.t1, p.n1)
    tms = G(t-p.td, p.K1, p.c1, p.t1, p.n1)
...
def G(t, K1, c1, t1, n1):
    return K1 * ((c1*(t - t1))**n1 * exp(-c1*(t - t1))) / ((n1**n1) * exp(-n1))


def H(t, M, T, alpha):
    """
    Retinal response
    exponential decay for temporal envelope (after Rodieck 1965)
    """
    return M + T*exp(-alpha*t)
